...
type AppOpenReq struct {
	Version string `json:"version" form:"version"` // 版本 v2
...
type AppProxyUserReq struct {
	...
	Password        string `json:"password" form:"password"`               //密码(不传随机生成)
	LimitFlow       int    `json:"limitFlow" form:"limitFlow"`             //动态流量上限
	MainUsername    string `json:"mainUsername" form:"mainUsername"`       //平台主账号  选填 平台主账号和渠道商主账号两个必填一个
	AppMainUsername string `json:"appMainUsername" form:"appMainUsername"` //渠道商主账号 选填 平台主账号和渠道商主账号两个必填一个
...
type AppGetOrderReq struct {
	OrderNo  string `json:"orderNo" form:"orderNo"`   // 平台订单编号
	Page     int    `json:"page" form:"page"`         // 页码  默认1
	PageSize int    `json:"pageSize" form:"pageSize"` // 每页显示数量  默认10 最大100
}
...
type AppInstanceOpenReq struct {
	AppOrderNo string      `json:"appOrderNo" form:"appOrderNo"` //购买者订单号(渠道商订单号)
	Params     []OpenParam `json:"params" form:"params"`         //购买代理产品列表
}

type OpenParam struct {
	ProductNo    string      `json:"productNo" form:"productNo"`       //商品编号（如果存在，后面6项无意义）
	...
	Unit         int8        `json:"unit"  form:"unit"`                //单位 1=天 2=周(7天) 3=月(自然月) 4=年(自然年365，366) 10=无限制
	IspType      int         `json:"ispType"  form:"ispType"`          //isp类型 1=单isp 2=双isp
	Count        int         `json:"count" form:"count"`               //购买数量 （实例个数）静态必填 默认1 一次最大20
	Duration     int32       `json:"duration" form:"duration"`         //必要 时长 默认1 为Unit的时长
	Renew        bool        `json:"renew"  form:"renew"`              //是否续费 1续费 默认0
	ExtBandWidth int32       `json:"extBandWidth" form:"extBandWidth"` //额外增加带宽 单位Mbps
	AppUsername  string      `json:"appUsername" form:"appUsername"`   //渠道商主账号，开通动态代理的时候必填(必须在平台上注册过)
	Flow         int         `json:"flow" form:"flow"`                 //动态流量 最大102400MB 动态必填 单位MB
	UseBridge    uint8       `json:"useBridge" form:"useBridge"`       //1=使用桥 2=不使用桥 3=随app设置 默认3
	CIDRBlocks   []CIDRBlock `json:"cidrBlocks" form:"cidrBlocks"`     //静态购买所在网段及数量（产品有的才支持） 2024/06/27新增
...
type AppInstanceRenewReq struct {
	AppOrderNo string     `json:"appOrderNo" form:"appOrderNo"` //购买者订单号(渠道商订单号)
	Instances  []Instance `json:"instances" form:"instances"`   //实例列表
}

type Instance struct {
	InstanceNo string `json:"instanceNo" form:"instanceNo"` //平台实例编号
